...
function processThread(thread,fromLoader){
	...
	var loadList=thread.f,
		...
		p=function(modName){
			...
			console.log('Process Module : '+modName);
	...
	console.log('loadList : ',loadList.join());
	console.groupEnd();
	//有需要加载的meta文件加载
	//否则attach模块
...
function attachMod(thread){
	...
	console.groupCollapsed('Completed Thread: '+thread.id);
	console.info(thread);
	console.groupEnd();
...
proto={
	...
	addFile:function(name,info){
		if(typeof name=='object'){
			...
			console.log('Add Meta File :: '+name);
			_meta_[name]=info;	
		}
		return this;
	},
